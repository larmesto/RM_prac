...
public:
    ...
    //
    int Update_VFH( double laser_ranges[360],
                    int current_speed,
	...
	int  GetHistSize() {return HIST_SIZE; }
	int GetSectorAngle() {return SECTOR_ANGLE;}
...
private:
    ...
    int Calculate_Cells_Mag( double laser_ranges[360], int speed );
    // Returns 0 if something got inside the safety distance, else 1.
    int Build_Primary_Polar_Histogram( double laser_ranges[360], int speed );
