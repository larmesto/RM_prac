...
class PublisherTrajectory : public rclcpp::Node
{
  public:
    ...
    : Node("NODE_NAME")
    {
      cmd_vel_publisher = this->create_publisher<"WRITE MSG TYPE">("WRITE_TOPIC_NAME", 10);

      odom_subscriber = this->create_subscription<"WRITE MSG TYPE">(
      "WRITE_TOPIC_NAME", 10, std::bind(&PublisherTrajectory::odom_data_callback, this,std::placeholders::_1));
  ...
  private:
    ...
    rclcpp::Publisher<"WRITE MSG TYPE">::SharedPtr cmd_vel_publisher;
    rclcpp::Subscription<"WRITE MSG TYPE">::SharedPtr odom_subscriber;
};
